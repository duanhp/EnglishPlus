...
uses
  Classes, {$IFDEF VER6P}Variants,{$ENDIF} SysUtils;
...
function UnicodeToUtf8(Dest: PAnsiChar; Source: PWideChar; MaxBytes: Integer): Integer;
begin
  Result := CLRClasses.UnicodeToUtf8(Dest, MaxBytes, Source, Cardinal(-1));
end;
...
function Utf8ToUnicode(Dest: PWideChar; Source: PAnsiChar; MaxChars: Integer): Integer;
begin
  Result := CLRClasses.Utf8ToUnicode(Dest, MaxChars, Source, Cardinal(-1));
end;
...
var
  UTF8: AnsiString;
begin
  UTF8 := AnsiToUtf8({$IFDEF VER7P}string{$ENDIF}(chars));
