...
begin
  ...
  if FDataSetService.IsFullRefresh or (FDataSet.ReadOnly and not FDataSetService.IsUniqueRecords) then begin
    ...
    if FCondSB.Length = 0 then
      FHeaderSB.Append(FDataSet.SQL.Text)
    else begin
      FHeaderSB.Append(_AddWhere(FDataSet.SQL.Text, FCondSB.ToString, TMSParser, False));
...
function TCustomMSDataSetService.GetRecCount: integer;
  ...
  begin
    ...
    try
      ...
      if IsCompactEdition(GetIConnection.ProviderPrimaryVer) then begin
        UQ.Open;
    ...
    try
      if Parser.ToLexem(lxSELECT) <> lcEnd then begin
        ...
        if Parser.ToLexem(lxFROM) <> lcEnd then begin
          ...
          if GetOLEDBProvider <> prCompact then begin
