...
{$IFDEF VER6P}
  FMTBcd,
{$ENDIF}
  DAConsts, SysUtils, ActiveX, SyncObjs, CRParser, CRAccess, MemData, MemUtils,
{$IFNDEF UNIDACPRO}
...
const
  ...
  prNativeClientVerison = prOleDBBase + 59; // TNativeClientVerison
...
{ TOLEDBConnection }
  ...
  TNativeClientVerison = (ncAuto, nc2008);
  ...
  {$ENDIF}
    ...
    FNativeClientVerison: TNativeClientVerison;
  ...
  public
    ...
    procedure Check(const Status: HRESULT; Component: TObject;
      AnalyzeMethod: TAnalyzeMethod = nil; const Arg: IntPtr = nil; Err: Exception = nil); virtual;
    procedure OLEDBError(const ErrorCode: HRESULT; Component: TObject;
      AnalyzeMethod: TAnalyzeMethod = nil; const Arg: IntPtr = nil; Err: Exception = nil);
...
{ TOLEDBCommand }
  ...
  { Rows }
    ...
    procedure Check(const Status: HRESULT; AnalyzeMethod: TAnalyzeMethod = nil; Err: Exception = nil); virtual;
  ...
  { Params }
    ...
    procedure GetNextResult(out ResultSet: IUnknown; out RowsAffected: NativeInt; Err: Exception = nil);
...
{ EMSError }
  ...
  function GetProviderName(const Provider: TOLEDBProvider; const CompactVersion: TCompactVersion = cvAuto;
    const NativeClientVerison: TNativeClientVerison = ncAuto): _string;
  ...
  function GetNativeClientVerison(const ProviderName: _string): TNativeClientVerison;
...
{$ENDIF}
{$ENDIF}
  {$IFNDEF UNIDACPRO}MSParser{$ELSE}MSParserUni{$ENDIF}, Math;
...
begin
  ...
  {$IFNDEF FPC}
    dtFmtBCD:
      if IsParam and not IsCompactEdition(ServerVersion) then
        Result := DBTYPE_STR
      else
        Result := DBTYPE_NUMERIC;
  {$ENDIF}
...
begin
  if (Status <> S_OK) and (Status <> DB_S_ERRORSOCCURRED) and ((FProvider <> prCompact) or (Status <> DB_S_MULTIPLECHANGES)) then
    OLEDBError(Status, Component, AnalyzeMethod, Arg, Err);
end;
...
begin
  ...
  case Prop of
    ...
    prNativeClientVerison:
      FNativeClientVerison := TNativeClientVerison(Value);
    prConnectionTimeout:
...
begin
  ...
  if Status <> S_OK then
    FConnection.Check(Status, Component, AnalyzeMethod, nil, Err);
end;
      ...
      try
        ...
        on E: EMSError do begin
          if (FIMultipleResults <> nil) and (ResultSet = nil) and (hr <> DB_S_NORESULT) then
            GetNextResult(ResultSet, RowsAffected, E);
          raise;
        end;
        on E: Exception do begin
          if (FIMultipleResults <> nil) and (ResultSet = nil) and (hr <> DB_S_NORESULT) and (Err <> nil) then
            GetNextResult(ResultSet, RowsAffected, Err)
          else
            raise;
        end;
      end;
...
function TOLEDBCommand.CreateProcCall(const Name: _string; NeedDescribe: boolean; IsQuery: boolean): _string;
  ...
  var
    Database, Owner, ProcName: _string;
...
begin
  ...
  i := Pos(';', Name);
  if i = 0 then
    FSQL := 'CALL ' + SQLInfo.NormalizeName(Name, _char('"'), _char('"'){May be bug in MS SQL Server}) + ' '
  else begin
    s := Name;
    s1 := Copy(Name, i, 1024);
    Delete(s, i, 1024);
    FSQL := 'CALL ' + SQLInfo.NormalizeName(s, _char('"'), _char('"'){May be bug in MS SQL Server}) + s1
  end;
              ...
              repeat
                ...
                while True do begin
                  ...
                  if {(Length(StLex) > 0) and (StLex[1] in ['[, ' + '], ' + '", ' + '.']) and}
                    ((CodeLexem <> 0) and (CodeLexem <> lcBlank)) or (BracketCount > 0) then begin
                    if StLex = '(' then
                      inc(BracketCount);
                    if StLex = ')' then
                      dec(BracketCount);
    ...
    for i := 0 to Cnt - 1 do
      with rgBindings[i] do begin
        ...
        if IsCompactEdition(DBMSPrimaryVer) then begin
          if (TOLEDBFieldDesc(Field).SubDataType = dtSingle) then // OLEDBType = DBTYPE_R4
            wType := DBTYPE_R4;
          if (TOLEDBFieldDesc(Field).SubDataType = dtUInt8) then begin // OLEDBType = DBTYPE_UI1
            wType := DBTYPE_UI1;
            cbMaxLen := SizeOf(Word);
          end;
          if (TOLEDBFieldDesc(Field).SubDataType = dtNumeric) then begin
        ...
        begin
          ...
          try
            ...
            {$ENDIF}
              ...
              dtDateTime:
                if (Provider = prCompact) and (FCursorType <> ctBaseTable) and (not GetNull(i + 1, pRec)) then begin
                {$IFDEF CLR}
                  DBTimeStamp := TDBTimeStamp(Marshal.PtrToStructure(pValue, TypeOf(TDBTimeStamp)));
                {$ELSE}
                  DBTimeStamp := PDBTimeStamp(pValue)^;
              ...
              dtCurrency:
                if (Provider = prCompact) and (FCursorType <> ctBaseTable) then begin
                {$IFDEF CLR}
...
const
  fmtGetNonSystemProcSQL =
    'SELECT'#$D#$A +
    '  CAST(db_name() AS VARCHAR(128)) AS PROCEDURE_CATALOG,'#$D#$A +
    '  CAST(%s(uid) AS VARCHAR(128)) AS PROCEDURE_SCHEMA,'#$D#$A +
    '  CAST(name AS VARCHAR(128)) AS PROCEDURE_NAME,'#$D#$A +
    '  CAST(NULL AS VARCHAR(1)) AS PROCEDURE_TYPE,'#$D#$A +
    '  crdate AS DATE_CREATED,'#$D#$A +
    '  CAST(NULL AS DATETIME) AS DATE_MODIFIED'#$D#$A +
    'FROM sysobjects'#$D#$A +
    'WHERE %s (OBJECTPROPERTY(id, N''IsProcedure'') = 1 or OBJECTPROPERTY(id, N''IsExtendedProc'') = 1)'#$D#$A +
    'ORDER BY uid, name';
var
  Catalog, ProcSchema, ProcName, Scope, WhereClause: _string;
  rgRestrictions: TRestrictions;
  Rowset: IRowset;
begin
  if FRecordSet.FCommand.FConnection.FProvider = prCompact then begin
  ...
  if Scope = 'LOCAL' then begin
    ...
    FRecordSet.SetSQL(_Format(fmtGetNonSystemProcSQL, [GetSchemaNameFunc, WhereClause]));
    FRecordSet.Open;
    Result := FRecordSet;
  end
  else begin
    ...
    Rowset := FRecordSet.FCommand.FConnection.GetSchemaRowset(DBSCHEMA_PROCEDURES, rgRestrictions);
    FRecordSet.SetIRowset(Rowset, False);
    FRecordSet.Open;
...
procedure TOLEDBMetaData.CopyProceduresData(Restrictions: _TStrings);
const
  snCATALOG       = 1;
  snSCHEMA        = 2;
  snPROC_NAME     = 3;
...
begin
  ...
  while FRecordSetHelper.NextRecord do begin
    FMemDataHelper.InitRecord;
    CopyRecord([snCATALOG, snSCHEMA, snPROC_NAME, snDATE_CREATED, snDATE_MODIFIED],
      [dnCATALOG, dnSCHEMA, dnPROC_NAME, dnDATE_CREATED, dnDATE_MODIFIED]);
    FMemDataHelper.AppendRecord;
  end;
end;
...
const
  ...
  dnSCALE      = 10;
var
  snCATALOG, snSCHEMA, snPROC_NAME, snPARAM_NAME, snPOSITION, snPARAM_TYPE, snDATA_TYPE,
    snLENGTH, snNUM_PRECISION, snNUM_SCALE: integer;
